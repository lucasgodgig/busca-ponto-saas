...
export const appRouter = router({
  ...
  tests: router({
    /**
     * Listar todos os testes com status
     * Apenas admin pode acessar
     */
    listAll: adminProcedure.query(async () => {
      return await db.getAllTestUsers();
    }),
    ...
    listActive: adminProcedure.query(async () => {
      return await db.getActiveTestUsers();
    }),

    /**
     * Listar testes expirados
     * Apenas admin pode acessar
     */
    listExpired: adminProcedure.query(async () => {
      return await db.getExpiredTestUsers();
    }),
    ...
    getStatistics: adminProcedure.query(async () => {
      return await db.getTestStatistics();
    }),

    /**
     * Converter teste para usuário pago
     * Apenas admin pode acessar
     */
    convertToPaid: adminProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          await db.convertTestUserToPaid(input.userId);
          return { success: true, message: "Usuário convertido para pago com sucesso" };
        } catch (error) {
          console.error("[Tests] Error converting test user:", error);
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Erro ao converter usuário de teste",
          });
        }
      }),

    /**
     * Obter informações de um teste específico
     * Apenas admin pode acessar
     */
    getById: adminProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (user.memberType !== "member_test") {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Usuário não é um teste",
          });
        }
        ...
        return {
          ...user,
          status: user.testExpiresAt && user.testExpiresAt > now ? "ativo" : "expirado",
          daysRemaining: user.testExpiresAt
            ? Math.ceil((user.testExpiresAt.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
            : 0,
        };
      }),
  }),
