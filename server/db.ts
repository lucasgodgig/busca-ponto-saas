...
export async function updateCommercialPointStatus(pointId: number, status: "pendente" | "aprovado" | "rejeitado") {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot update point status: database not available");
    return null;
  }
  ...
  try {
    ...
    console.log("[Database] Point status updated successfully");
    return result;
  } catch (error) {
    console.error("[Database] Failed to update point status:", error);
    throw error;
  }
}

export async function getCommercialPointsByRequestIdAndStatus(requestId: number, status?: "pendente" | "aprovado" | "rejeitado") {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot get commercial points: database not available");
    return [];
  ...
  try {
    const conditions = [eq(commercialPoints.requestId, requestId)];
    if (status) {
      conditions.push(eq(commercialPoints.status, status));
    }
...
export async function getCommercialPointOptions(requestId: number) {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot get point options: database not available");
    return [];
  }
  ...
  try {
    ...
    console.error("[Database] Failed to get point options:", error);
    throw error;
  }
}
